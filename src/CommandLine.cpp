...
void CommandLine::setFileName () {
    int countSlash = StringHelper::count(CommandLine::openedFilePath,'/');
    int countBackslash = StringHelper::count(CommandLine::openedFilePath, (char)92);

    if(countSlash == 0 && countBackslash == 0) CommandLine::fileName = CommandLine::openedFilePath;
    else if (countSlash > 0) {
        std::string* sepPath = StringHelper::split(openedFilePath,'/');
        CommandLine::fileName = sepPath[countSlash];
    }
    else if (countBackslash > 0) {
        std::string* sepPath = StringHelper::split(openedFilePath,(char)92);
        CommandLine::fileName = sepPath[countBackslash];
    }
}
...
void CommandLine::save (int id, std::string path) {
    if (openedFile) {
...
void CommandLine::deterministic (int id) {
    if (openedFile) {
        if(id >= 1 && id <= AutomatonList::automatons.size()) {
...
void CommandLine::recognize  (int id, std::string word) {
    if (openedFile) {
...
void CommandLine::_union (int id1, int id2) {
    if (openedFile) {
...
void CommandLine::concat (int id1, int id2) {
    if (openedFile) {
        if((id1 >= 1 && id1 <= AutomatonList::automatons.size()) && (id2 >= 1 && id2 <= AutomatonList::automatons.size())) {
...
void CommandLine::reg (std::string regex) {
    if (openedFile) {
...
void CommandLine::determine (int id) {
    if (openedFile) {
        if(id >= 1 && id <= AutomatonList::automatons.size()) {
...
void CommandLine::isFiniteLang (int id) {
    if (openedFile) {
        if(id >= 1 && id <= AutomatonList::automatons.size()) {
